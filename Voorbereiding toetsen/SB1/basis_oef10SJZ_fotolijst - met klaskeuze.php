<?php
  ...
  if(isset($_POST["btnKlas"])){
    ...
    // foto's van leerlingen uit juiste klas weergeven
    foreach($leerlingen as $leerlingID => $fotoLeerling) {
      $klasLeerling = substr($leerlingID, strpos($leerlingID, '_') + 1, 3);
			$naamVanLeerling = substr($leerlingID, 0, strpos($leerlingID, "_"));
     
      if($klasLeerling == $gekozenklas){
        // $output .= "<div class='persoon'><img src='fotosSJZ/$fotoLeerling.jpg'></div>";
        $output .= "<div class='persoon'>";
				$output .= "<img src='fotosSJZ/$fotoLeerling.JPG' alt='foto van $naamVanLeerling'> <div class='naam'><p>$naamVanLeerling</p></div>";
				$output .= "</div>";
      }
    }
  }
